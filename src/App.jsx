...
function HostDashboard({ room, players, onReveal, onNext, onCorrect, onBuzzerCorrect }) {
  ...
  if (room.status === 'lobby') return (
    <div className="text-center py-20 space-y-12">
      ...
      <div className="bg-white p-8 rounded-3xl border border-sky-100 max-w-4xl mx-auto shadow-xl">
        ...
        <div className="flex flex-wrap gap-4 justify-center">{players.map(p=><span key={p.id} className="bg-slate-50 px-6 py-3 rounded-full text-xl font-semibold shadow-sm text-slate-600">{p.name}</span>)}</div>
      </div>
  ...
  if (room.status === 'finished') return (
    <div className="space-y-8 max-w-4xl mx-auto py-10 text-slate-700">
...
function PlayerDashboard({ room, player, onAnswer, onBuzz }) {
  ...
  if (room.status === 'finished') return <div className="text-center py-20 bg-white rounded-3xl border border-sky-100 shadow-xl max-w-sm mx-auto text-slate-700"><Trophy size={64} className="mx-auto text-[#E69F00] mb-6"/><div className="text-6xl font-bold">{player.score}</div></div>;
